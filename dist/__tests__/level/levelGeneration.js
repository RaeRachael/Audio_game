...
describe("function createLevel()", function () {
    it("creates an array the size of the argument squared or greater", function () {
        expect(level_1.createLevel(1).length).toBeGreaterThan(0);
        expect(level_1.createLevel(2).length).toBeGreaterThan(3);
    });
    it("creates tiles with postions and availiable paths", function () {
        expect(Number.isInteger(level_1.createLevel(1)[0].position.x)).toEqual(true);
        expect(Number.isInteger(level_1.createLevel(1)[0].position.x)).toEqual(true);
        expect(typeof level_1.createLevel(1)[0].paths.North === 'boolean').toBeTruthy();
        expect(typeof level_1.createLevel(1)[0].paths.South === 'boolean').toBeTruthy();
        expect(typeof level_1.createLevel(1)[0].paths.East === 'boolean').toBeTruthy();
        expect(typeof level_1.createLevel(1)[0].paths.West === 'boolean').toBeTruthy();
    });
    it("the first tile is at '0,0' and has two availiable path", function () {
        var firstTile = level_1.createLevel(2)[0];
    ...
    it("builds tiles from the nearest open path", function () {
        var levelMap = level_1.createLevel(2);
    ...
    it("adds a new OpenPath when needed", function () {
        ...
        var levelMap = level_1.createLevel(4);
